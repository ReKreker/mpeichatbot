...
from data.cb_data import PractChooseCbFactory, PractListCbFactory
...
async def get_pract_name(msg: types.Message, state: FSMContext) -> None:
    ...
    test_names = ["Практика 1", "Практика 2", "Практика 3"]
    practicies = InlineKeyboardBuilder()
    for i in range(len(test_names)):
        data = PractListCbFactory(user_id=msg.from_user.id, reply_msg_id=msg.message_id, name=test_names[i])
        practicies.add(types.InlineKeyboardButton(text=test_names[i], callback_data=data.pack()))
    practicies.adjust(1)
    await msg.reply("Выбери, какую сдать практику:", reply_markup=practicies.as_markup())
...
async def send_info(query: types.CallbackQuery, state: FSMContext) -> None:
    ...
    data = PractListCbFactory.unpack(query.data)
    await state.set_data({"name": data.name})
    await state.set_state(Practice.to_upload)
...
async def forward(msg: types.Message, state: FSMContext, bot: Bot) -> None:
    ...
    no_data = PractChooseCbFactory(user_id=msg.from_user.id, reply_msg_id=msg.message_id, is_yes=False)
    builder.add(
        types.InlineKeyboardButton(
            text="🚫",
            callback_data=no_data.pack()
        ))
    yes_data = PractChooseCbFactory(user_id=msg.from_user.id, reply_msg_id=msg.message_id, is_yes=True)
    builder.add(
        types.InlineKeyboardButton(
            text="✅",
    ...
    await state.set_state(Practice.forwarded)
